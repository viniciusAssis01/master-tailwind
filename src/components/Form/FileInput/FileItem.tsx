...
const fileItem = tv({
	...
	variants: {
		state: {
			...
			complete: { container: "border-violet-500 dark:border-violet-300/40" },
			error: {
				container:
				...
				deleteButton:
					"text-error-700 hover:text-error-900 dark:text-error-400 dark:hover:text-err3",
			},
		},
	},
...
export interface IFileItemProps extends VariantProps<typeof fileItem> {
	name: string;
	size: number;
}
